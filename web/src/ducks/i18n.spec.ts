  ...
  describe('actions', () => {
    describe('setLocaleByBrowserLanguages', () => {
      it('returns ja locale if not found', () => {
        expect(actions.setLocaleByBrowserLanguages(['fr']).payload.locale).toBe(
          'ja'
        )
      })
      it('returns first found locale', () => {
        expect(
          actions.setLocaleByBrowserLanguages(['en', 'ja']).payload.locale
        ).toBe('en')
      })
      it('handle en-US, ja-JP', () => {
        expect(
          actions.setLocaleByBrowserLanguages(['en-US', 'ja-JP']).payload.locale
        ).toBe('en')
      })
    })
    describe('setLocale', () => {
